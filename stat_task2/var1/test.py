...
def score(test_stat: dict) -> int:
    ...
    if test_stat[0.99][1000]["mean_error"] < 0.02 and test_stat[0.99][1000]["mean_interval_length"] < 0.004:
        total_score += 1

    if test_stat[0.9][1000]["mean_error"] < 0.1 and test_stat[0.9][1000]["mean_interval_length"] < 0.0021:
        total_score += 1

    if test_stat[0.7][100]["mean_error"] < 0.31 and test_stat[0.7][100]["mean_interval_length"] < 0.00415:
        total_score += 1

    if test_stat[0.9][100]["mean_error"] < 0.1 and test_stat[0.9][100]["mean_interval_length"] < 0.00658:
        total_score += 1

    if test_stat[0.95][10]["mean_error"] < 0.14 and test_stat[0.95][10]["mean_interval_length"] < 0.025:
        total_score += 1

    if test_stat[0.9][10]["mean_error"] < 0.11 and test_stat[0.9][10]["mean_interval_length"] < 0.0206:
        total_score += 1
...
def test(solution) -> int:
    data = pd.read_csv("stat_task2/var1/sample.csv")
    a_sample = data["a"]
