...
hyp_problem1_variant4 = ProblemVariant(code="hyp_task1_var4",
                                       input_data_text="""
                                       Четыре целых числа:
...
class TransformerHypProblem1Variant4(VariantTransformer):
    def __init__(self, code, input_data_text, output_data_text):
        ...
        self.initial_p = Decimal("0.06")
        self.positive_delta_p = Decimal("0.3")
        self.negative_delta_p = Decimal("0.02")
    ...
    def _get_transformed_random_state(self, random_state):
        ...
        alpha_numerator = min_alpha_numerator + ((random_state - 24) % (max_alpha_numerator - min_alpha_numerator + 1))
    ...
    def get_sample(self, iter_size, sample_size, random_state, delta_factor=0):
        transformed_random_state = (random_state - 89) % 4512
    ...
    def get_description(self, random_state):
        ...
        problem_text = f"""
        В рамках теста мы хотим попробовать
        заменить операторов на продажного робота
        и проверить, не станет ли робот 
        продавать лучше операторов.
        Составьте критерий
        с уровнем значимости ${alpha}$,
        который позволит по данным теста
        принять решение о замены оператора на робота.
        """
    ...
    def get_solution_description(self, random_state):
        ...
        В это тесте нас интересует только
        лучшая работа робота, чем оператора.
        Здесь $H_0: p_c \geq p_t$, $H_1: p_c < p_t$,
