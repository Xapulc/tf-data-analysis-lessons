...
from scipy.stats import expon, gamma, anderson_ksamp
from tools import ProblemVariant, VariantTransformer


hyp_problem2_variant4 = ProblemVariant(code="hyp_task2_var4",
                                       input_data_text="""
...
class TransformerHypProblem2Variant4(VariantTransformer):
    def __init__(self, code, input_data_text, output_data_text):
        ...
        self.null_dist = expon(scale=1)
        self.first_dist = expon(scale=1.3)
        self.second_dist = gamma(a=1.2)
        self.third_dist = gamma(a=1, loc=-0.1)
    ...
    def get_example_sample(self, iter_size, sample_size, random_state):
        init_random_state = (random_state - 542) % 4235 + 4124
        sample_list = [
            {"dist_num": i,
    ...
    def get_sample(self, iter_size, sample_size, random_state, y_dist_num=0):
        init_random_state = (random_state - 553) % 4235 + 4124
        if y_dist_num == 1:
